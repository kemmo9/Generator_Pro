...
import tempfile
import shutil
import whisper # <<< NEW: Import for word-level timestamps

# --- Configuration & Asset URLs ---
if not hasattr(PIL.Image, 'ANTIALIAS'): PIL.Image.ANTIALIAS = PIL.Image.LANCZOS

ASSET_URLS = {
    # Using the existing template with pre-built awards as requested
...
# For now, we will use a single subtitle style that gets its size modified
SUBTITLE_STYLES = { "standard": {"font": "Arial-Bold", "color": "white", "stroke_color": "black", "stroke_width": 2.5} }
...
def download_file_to_temp(url, temp_dir): filename = os.path.join(temp_dir, os.path.basename(url.split("?")[0])); with requests.get(url, stream=True) as r: r.raise_for_status(); open(filename, 'wb').write(r.content); return filename
...
# --- NEW: Whisper Timestamp Generation ---
def get_word_timestamps(audio_path):
    update_job_progress("Transcribing for timestamps...")
    model = whisper.load_model("tiny.en") # Use a small, fast model
    result = model.transcribe(audio_path, word_timestamps=True)
    return result["segments"]

# --- Reddit Post Image Generation (Updated) ---
def create_reddit_post_image(data: dict, temp_dir: str):
    ...
    # The "Posted by" is already on the template image, so we just draw the name and subreddit
...
def create_reddit_video_task(reddit_data: dict, options: dict):
    ...
    try:
        ...
        full_text = f"{title_text}. {body_text}" if body_text.strip() else title_text

        audio_path = os.path.join(temp_dir, f"full_audio_{job_id}.mp3")
        generate_audio_elevenlabs(full_text, audio_path, VOICE_IDS[narrator_voice])
        
        segments = get_word_timestamps(audio_path)
        title_word_count = len(title_text.split())
        title_duration = segments[0]['words'][title_word_count-1]['end'] if title_word_count > 0 else 0
        ...
        background_video = VideoFileClip(bg_path).subclip(0, audio_clip.duration).set_audio(audio_clip)

        update_job_progress("Generating dynamic subtitles...")
        post_image_path = create_reddit_post_image(reddit_data, temp_dir)
        post_clip = ImageClip(post_image_path).set_duration(title_duration).resize(width=1000).set_position('center')
        
        subtitle_clips = []
        word_group_size = options.get("word_group_size", 3)
        size_multiplier = options.get("subtitle_size_multiplier", 1.0)
        base_style = SUBTITLE_STYLES["standard"]
        font_size = int(base_style['fontsize'] * size_multiplier)

        all_words = [word for seg in segments for word in seg['words']]
        body_words = all_words[title_word_count:]
        
        for i in range(0, len(body_words), word_group_size):
            chunk = body_words[i:i+word_group_size]
            text = " ".join([word_info['word'] for word_info in chunk])
            start_time = chunk[0]['start'] - title_duration
            end_time = chunk[-1]['end'] - title_duration
            
            txt_clip = TextClip(text, fontsize=font_size, font=base_style['font'], color=base_style['color'], 
                                stroke_color=base_style['stroke_color'], stroke_width=base_style['stroke_width'],
                                size=(background_video.w * 0.85, None), method='caption'
        ...
        body_start_time = title_duration if title_duration > 0 else 0
        ...
        final_video.write_videofile(output_path, codec="libx264", audio_codec="aac", fps=24, threads=2)
...
def create_video_task(dialogue_data: list, options: dict):
    job_id, temp_dir = get_current_job().id, tempfile.mkdtemp()
    audio_clips, composited_clips = [], []
    try:
        update_job_progress("Downloading assets...")
        char_paths = {"peter": download_file_to_temp(ASSET_URLS["peter_char"], temp_dir), "brian": download_file_to_temp(ASSET_URLS["brian_char"], temp_dir)}
        
        style = SUBTITLE_STYLES["standard"]; size_multiplier = options.get("subtitle_size_multiplier", 1.0); style['fontsize'] = int(style['fontsize'] * size_multiplier)
        bg_url = BACKGROUND_VIDEO_URLS.get(options.get("backgroundVideo", "minecraft_parkour1"))
        ...
        bg_path = download_file_to_temp(bg_url, temp_dir)
        background_clip = VideoFileClip(bg_path).subclip(0, final_audio.duration).set_audio(final_audio); composited_clips.append(background_clip)
        
        update_job_progress("Compositing video...")
        current_time = 0
        for i, line_data in enumerate(dialogue_data):
            img_clip = ImageClip(char_paths[line_data["character"]]).set_duration(audio_clips[i].duration).set_start(current_time).resize(height=600).set_position('center')
        ...
        update_job_progress("Rendering..."); final_video.write_videofile(output_path, codec="libx264", audio_codec="aac", fps=24)
        update_job_progress("Uploading..."); upload_result = cloudinary.uploader.upload(output_path, resource_type="video")
