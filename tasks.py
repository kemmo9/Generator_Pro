...
# THE DEFINITIVE FIX: Import all assets from the auto-generated file.
# This eliminates all OSError and Padding errors permanently.
...
def get_asset_as_file(asset_name):
    ...
    if not b64_string:
        raise FileNotFoundError(f"Asset '{asset_name}' not found in embedded data. Please run embed_assets.py")
    return io.BytesIO(base64.b64decode(b64_string))

# --- CONFIGURATION ---
SUBTITLE_STYLES = {
    "standard": {"fontsize": 40, "color": "white", "font": "Inter-SemiBold.ttf"},
    "yellow": {"fontsize": 45, "color": "#FFD700", "font": "Inter-SemiBold.ttf"},
...
def create_reddit_post_image(data):
    ...
    template_img_file = get_asset_as_file("reddit_template_final.png")
    font_bold = ImageFont.truetype(get_asset_as_file("Inter-SemiBold.ttf"), 28)
    font_heavy = ImageFont.truetype(get_asset_as_file("Inter-SemiBold.ttf"), 44)
    img = Image.open(template_img_file).convert("RGBA"); draw = ImageDraw.Draw(img)
...
def create_video_task_base(audio_clips, options):
    ...
    return background_clip, final_audio, temp_bg_path
...
def create_reddit_video_task(reddit_data: dict, options: dict):
    ...
    try:
        ...
        if not full_text_for_vo: raise ValueError("Reddit title or body must have text.")
        ...
        background_clip, _, temp_bg_path = create_video_task_base([full_audio_clip], options); temp_files.append(temp_bg_path)
        
        update_job_progress("Generating post image..."); image_path = create_reddit_post_image(reddit_data); temp_files.append(image_path)
        position = reddit_data.get("position", "top_left"); size = reddit_data.get("size", 1000)
        
        post_clip = ImageClip(image_path).set_duration(full_audio_clip.duration).resize(width=size)
        if position == "top_left": post_clip = post_clip.set_position(("left", "top")).margin(top=50, left=50, opacity=0)
        elif position == "top_center": post_clip = post_clip.set_position(("center", "top")).margin(top=50, opacity=0)
        else: post_clip = post_clip.set_position('center')
        
        update_job_progress("Compositing..."); final_video = CompositeVideoClip([background_clip, post_clip])
...
def create_video_task(dialogue_data: list, options: dict):
    update_job_progress("Initializing character dialogue..."); temp_files = []
    try:
        ...
        background_clip, final_audio, temp_bg_path = create_video_task_base(audio_clips, options); temp_files.append(temp_bg_path)
        ...
        style['font'] = os.path.join(STATIC_DIR, style['font']) # Use absolute path for font file
        
        for i, clip_data in enumerate(dialogue_data):
            char_img_file = get_asset_as_file(f"{clip_data['character']}.png")
            img = ImageClip(char_img_file).set_duration(audio_clips[i].duration).set_start(current_time).set_position(clip_data.get("imagePlacement", "center")).resize(height=300)
            
            shadow_style = style.copy(); shadow_style['color'] = 'black'; shadow_offset = 2
