...
def create_reddit_post_image(data: dict, temp_dir: str):
    ...
    template = PILImage.open(template_path).convert("RGBA"); pfp = PILImage.open(pfp_path).convert("RGBA").resize((68, 68))
    ...
    font_user = ImageFont.truetype(font_user_path, 26); font_title = ImageFont.truetype(font_title_path, 44)
    ...
    template.save(output_filename, "PNG"); return output_filename
...
def create_reddit_video_task(reddit_data: dict, options: dict):
    ...
    try:
        ...
        narrator_voice = options.get("narrator_voice", "reddit_default"); title_text, body_text = reddit_data.get('title', ''), reddit_data.get('body', ''); full_text = f"{title_text}. {body_text}" if body_text.strip() else title_text
        ...
        update_job_progress("Generating dynamic subtitles..."); post_image_path = create_reddit_post_image(reddit_data, temp_dir); post_clip = ImageClip(post_image_path).set_duration(title_duration).resize(width=1000).set_position('center')
        
        subtitle_clips = []
        word_group_size = options.get("word_group_size", 3); size_multiplier = options.get("subtitle_size_multiplier", 1.0)
        base_style = SUBTITLE_STYLES["standard"]; font_size = int(base_style['fontsize'] * size_multiplier)
        ...
        for i in range(0, len(body_words), word_group_size):
            ...
            txt_clip = TextClip(text, fontsize=font_size, font=base_style['font'], color=base_style['color'], stroke_color=base_style['stroke_color'], stroke_width=base_style['stroke_width'], size=(background_video.w * 0.85, None), method='caption').set_position(('center', 'center')).set_start(start_time).set_duration(end_time - start_time)
