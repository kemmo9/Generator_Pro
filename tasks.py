...
# --- THE NEW, ROBUST REDDIT VIDEO TASK ---
def create_reddit_video_task(reddit_data: dict, options: dict):
    update_job_progress("Initializing Reddit video..."); temp_files = []
    try:
        # Step 1: Save the image data received from the frontend
        image_data_url = reddit_data.get('image_data_url')
        if not image_data_url:
            raise ValueError("No image data was received from the frontend.")
        
        # Decode the Base64 image data sent from the browser
        ...
        # Step 2: Generate voiceover for the title
        ...
        generate_audio_elevenlabs(title_text, vo_filename, VOICE_IDS.get("reddit"))
        audio_clip = AudioFileClip(vo_filename)
        
        # Step 3: Composite the video
        update_job_progress("Compositing video...")
        reddit_post_clip = ImageClip(image_path).set_duration(audio_clip.duration).resize(width=1000).set_position('center')
