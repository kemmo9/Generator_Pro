...
def create_reddit_video_task(reddit_data: dict, options: dict):
    ...
    try:
        ...
        if not full_text_for_vo:
            raise ValueError("Reddit title or body must have text for voiceover.")
        ...
        final_video = CompositeVideoClip([background_clip, reddit_post_clip]).set_audio(full_audio_clip)
