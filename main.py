...
# Import the premium styles set from tasks to enforce monetization rules
...
# Mount static files directory (for CSS, images, etc.) and configure Jinja2 templates
...
async def create_checkout_session(request: Request, payload: dict = Body(...), user: dict = Depends(get_user)):
    ...
    try:
        checkout_session = stripe.checkout.Session.create(
            ...
            client_reference_id=user['sub'] # This links the Stripe transaction to the Auth0 user ID
        )
...
async def stripe_webhook(request: Request):
    """Listens for successful payments from Stripe to update user tiers."""
    ...
    if event['type'] == 'checkout.session.completed':
        ...
        # TODO: This is where you will add logic to call the Auth0 Management API
        # to update the user's app_metadata with their new tier.
...
async def queue_video_task(request: Request, payload: dict = Body(...), user: dict = Depends(get_user)):
    """Validates the request and queues a video generation task."""
    if not user:
        raise HTTPException(status_code=401, detail="Not authenticated")
    
    options_data = payload.get("options", {})
    selected_style = options_data.get("subtitleStyle")
    # This is a placeholder until the webhook logic is complete.
    user_tier = user.get("https://makeaclip.pro/tier", "free") 

    # Enforce premium style monetization
    ...
    # Route the job to the correct task function based on the selected template
    template = options_data.get("template")
    if template == "reddit":
        job = q.enqueue('tasks.create_reddit_video_task', payload.get("reddit_data", {}), options_data, job_timeout='5m')
    elif template == "character":
        job = q.enqueue('tasks.create_video_task', payload.get("dialogue_data", []), options_data, job_timeout='5m')
    else:
...
async def get_job_status(job_id: str):
    """Checks the status of a background video generation job."""
...
async def health_check():
    """A simple health check endpoint that Render uses to see if the app is alive."""
    return {"status": "ok"}
